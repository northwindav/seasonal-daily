...
from utils import (
    ...
    setup_map_axes,
    draw_province_boundaries,
    configure_colorbar,
    add_map_footer,
    save_and_close_map,
    plot_contourf_data,
)
from config_loader import get_region_config, list_regions, get_bounds_dict, get_projection_from_region
...
# - projection: cartopy projection object for the map
def create_fwi_map(fwi_data, lat, lon, date_str, filename, ontario_bounds=None, model_name=None, init_date=None, provinces=None, projection=None):
...
# - region_name (str, optional): Name of region for filename inclusion
# - projection_config (dict, optional): Projection configuration for mapping
# - target_date (datetime, optional): If specified, only generate map for this date
def generate_daily_maps(input_dir='input', output_dir='output/daily', region_bounds=None, province_names=None, buffer_km=150, region_name=None, projection_config=None, target_date=None):
    ...
    for filename in nc_files:
        ...
        for tidx, date_obj in enumerate(date_objs):
            ...
            if start_date <= date_obj <= end_date:
                ...
                output_file = os.path.join(output_dir, f"{model_name}_{region_normalized}_{date_str}_FWI_map.png")
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate daily FWI maps from CFS Seasonal Forecast ensembles',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='Examples:\n'
               '  python generate_daily_maps.py                                    # Use default region (Canada-wide)\n'
               '  python generate_daily_maps.py --region ontario                  # Generate for Ontario\n'
               '  python generate_daily_maps.py --region quebec --date 2026-05-06  # Single date test\n'
               '  python generate_daily_maps.py --list-regions                     # Show available regions'
    )
    parser.add_argument(
        '--region',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Generate map for a specific date (format: YYYY-MM-DD). Useful for testing.'
    )
    parser.add_argument(
        '--list-regions',
        action='store_true',
    ...
    parser.add_argument(
        ...
        help='Directory to save output maps (default: output/daily)'
    )
    ...
    try:
        ...
        if args.date:
            try:
                ...
                print(f"Error: Invalid date format '{args.date}'. Use YYYY-MM-DD", file=__import__('sys').stderr)
                exit(1)
